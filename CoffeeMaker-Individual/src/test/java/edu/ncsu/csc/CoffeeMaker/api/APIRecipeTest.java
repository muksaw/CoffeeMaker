...
public class APIRecipeTest {
    ...
    public void testDeleteRecipe () throws Exception {
        ...
        /* Simply will create a recipe and then save it */
        final Recipe r1 = createRecipe( "Coffee", 50, 3, 1, 1, 0 );
        service.save( r1 );

        /* Ensure that the recipe is there */
        Assertions.assertEquals( 1, service.count(), "Created one recipe in the database" );

        /* Now we will try to delete it */
    ...
    public void testEditRecipe () throws Exception {
        ...
        final Recipe r1 = createRecipe( "Coffee", 50, 3, 1, 1, 0 );
        service.save( r1 );
        final Recipe r4 = createRecipe( "Coffee", 75, 0, 2, 1, 2 );
        final Recipe r5 = createRecipe( "Coff33ee", 75, 0, 2, 1, 2 );

        /* Ensure that the recipe is there */
        Assertions.assertEquals( 1, service.count(), "Created one recipe in the database" );

        /* Now we will try to edit it */
        mvc.perform( put( "/api/v1/recipes" ).contentType( MediaType.APPLICATION_JSON )
                .content( TestUtils.asJsonString( r4 ) ) );

        mvc.perform( put( "/api/v1/recipes" ).contentType( MediaType.APPLICATION_JSON )
                .content( TestUtils.asJsonString( r5 ) ) ).andExpect( status().isConflict() );
    ...
    public void testDeleteNotExisting () throws Exception {
        ...
        /* Ensure that the recipe is there */
        Assertions.assertEquals( 0, service.count(), "The single recipe is now deleted" );

        /* Try to delete it again now that nothing is there */
        mvc.perform( delete( String.format( "/api/v1/recipes/%s", "Coffee" ) ).contentType( MediaType.APPLICATION_JSON )
