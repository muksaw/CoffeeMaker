...
public class APIRecipeController extends APIController {
    ...
    public ResponseEntity editRecipe ( @RequestBody final Recipe recipe ) {
        final Recipe existingRecipe = service.findByName( recipe.getName() );
        if ( null == existingRecipe ) {
            return new ResponseEntity( errorResponse( "Recipe with the name " + recipe.getName() + " doesn't exist." ),
                    HttpStatus.CONFLICT );
        }
