...
public class APIInventoryController extends APIController {
    ...
    public ResponseEntity updateInventory ( @RequestBody final Inventory inventory ) {
        ...
        for ( final Ingredient h : inventory.getIngredients() ) {
            inventoryCurrent.addToIngredient( h.getIngredient(), inventory.getIngredient( h.getIngredient() ) );
        }
     ...
     * REST API endpoint to provide update access to CoffeeMaker's singleton
     * Inventory. This will update the Inventory of the CoffeeMaker by adding
     * new arbitrary ingredients
     *
     * @param name
     *            contains the name of new ingredient
     * 
     * @param amount
     *
     *            contains the amount of the new ingredient
     * 
     * @return response to the request
     */
    @PostMapping ( BASE_PATH + "/inventory/{name}/{amount}" )
    public ResponseEntity addIngredient ( @PathVariable ( "name" ) final String name,
            @PathVariable ( "amount" ) final Integer amount ) {
        final Inventory inventoryCurrent = service.getInventory();
        if ( !inventoryCurrent.addIngredient( name, amount ) ) {
